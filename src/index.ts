...
class EventEmitter {
  private handles: { [type in Event]?: Array<() => void> } = {
    //
  };
...
export default class Gesture extends EventEmitter {
  ...
  private __privateTouchstart: any;
  private __privateTouchmove: any;
  private __privateTouchend: any;
  private __privateTouchcancel: any;
  ...
  constructor(selector: HTMLElement | string) {
    ...
    const target = (this.target =
      selector instanceof HTMLElement
        ...
        : null); // 获取目标元素
    this._init();
    this.__privateTouchstart = this._touchstart.bind(this);
    this.__privateTouchmove = this._touchmove.bind(this);
    this.__privateTouchend = this._touchend.bind(this);
    this.__privateTouchcancel = this._touchcancel.bind(this);
    // 绑定基本事件，需要注意this的指向，事件的处理方法均在prototype实现
    target.addEventListener("touchstart", this.__privateTouchstart, false);
    target.addEventListener("touchmove", this.__privateTouchmove, false);
    target.addEventListener("touchend", this.__privateTouchend, false);
    target.addEventListener("touchcancel", this.__privateTouchcancel, false);
  }
  private _touchstart(e) {
    ...
    if (e.touches.length > 1) {
      ...
      this.doubleTap =
        this.pretouch.timestamp &&
        now - this.pretouch.timestamp < DoubleTapTimeout && // 距离上一次时间间隔小于300ms
        ...
        Math.abs(this.touch.timestamp - this.pretouch.timestamp) <
          DoubleTapTimeout;
      this.pretouch = {
  ...
  private _touchend(e) {
    ...
    if (
      ...
      if (!this.doubleTap && timestamp - this.touch.timestamp < 300) {
        // 单次点击300ms内离开，触发点击事件
  ...
  public destroy() {
    ...
    const target = this.target;
    target.removeEventListener("touchstart", this.__privateTouchstart);
    target.removeEventListener("touchmove", this.__privateTouchmove);
    target.removeEventListener("touchend", this.__privateTouchend);
    target.removeEventListener("touchcancel", this.__privateTouchcancel);
    return false;
  }
}
